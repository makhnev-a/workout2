...
export const createNewWorkoutLog = asyncHandler(async (req, res) => {
	...
	if (workout) {
		...
		const exLogIds = createdExLog.map(log => log._id)
		const foundExerciseLog = await ExerciseLog.findById(workoutLog._id)

		foundExerciseLog.exerciseLogs = exLogIds

		const updatedWorkoutLog = await foundExerciseLog.save()
